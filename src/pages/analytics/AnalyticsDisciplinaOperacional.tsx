          ...
          <div className={`bg-card border rounded-xl p-4 ${selectedMes ? "border-[#FF5722]/30" : "border-border/50"}`}>
            ...
            <ResponsiveContainer width="100%" height={280}>
              <ComposedChart data={qualidadeComHeadcount} onClick={(e: any) => {
                ...
                <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 10 }} domain={[0, rightDomainMax]} ticks={(() => { const step = rightDomainMax <= 10 ? 2 : rightDomainMax <= 50 ? 10 : rightDomainMax <= 200 ? 50 : rightDomainMax <= 1000 ? 200 : rightDomainMax <= 5000 ? 1000 : rightDomainMax <= 20000 ? 5000 : 10000; const arr: number[] = []; for (let v = 0; v <= rightDomainMax; v += step) arr.push(v); if (arr[arr.length - 1] < rightDomainMax) arr.push(rightDomainMax); return arr.slice(0, 6); })()} tickFormatter={v => v >= 1000 ? `${(v/1000).toFixed(v >= 10000 || v % 1000 === 0 ? 0 : 1)}K` : `${v}`} label={{ value: "Headcount", angle: 90, position: "insideRight", style: { fontSize: 10, fill: "hsl(var(--muted-foreground))" }, offset: 0 }} />
